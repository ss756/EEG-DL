    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        ...
        for step in range(1, num_steps + 1):
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                # summary.value.add(tag='T1_Precision_numpy',  simple_value=T1_Precision_numpy)
                # summary.value.add(tag='T1_Recall_numpy',     simple_value=T1_Recall_numpy)
                # summary.value.add(tag='T1_F_Score_numpy',    simple_value=T1_F_Score_numpy)
                summary.value.add(tag='T2_accuracy_numpy',   simple_value=T2_accuracy_numpy)
                # summary.value.add(tag='T2_Precision_numpy',  simple_value=T2_Precision_numpy)
                # summary.value.add(tag='T2_Recall_numpy',     simple_value=T2_Recall_numpy)
                # summary.value.add(tag='T2_F_Score_numpy',    simple_value=T2_F_Score_numpy)
                summary.value.add(tag='T3_accuracy_numpy',   simple_value=T3_accuracy_numpy)
                # summary.value.add(tag='T3_Precision_numpy',  simple_value=T3_Precision_numpy)
                # summary.value.add(tag='T3_Recall_numpy',     simple_value=T3_Recall_numpy)
                # summary.value.add(tag='T3_F_Score_numpy',    simple_value=T3_F_Score_numpy)
                summary.value.add(tag='T4_accuracy_numpy',   simple_value=T4_accuracy_numpy)
                # summary.value.add(tag='T4_Precision_numpy',  simple_value=T4_Precision_numpy)
                # summary.value.add(tag='T4_Recall_numpy',     simple_value=T4_Recall_numpy)
                # summary.value.add(tag='T4_F_Score_numpy',    simple_value=T4_F_Score_numpy)
